...
class _HomeScreenState extends State<HomeScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Hiba a garanciális csekkek betöltésekor: $e')),
        );
      }
    }
  }
  ...
  Future<void> _clearAllData() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Biztosan törölni szeretnéd az összes garanciális csekket? Ez a művelet nem vonható vissza!'),
        actions: [
          TextButton(
  ...
  Future<void> _deleteGuaranteeCheck(GuaranteeCheck check) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Garanciális csekk törlése'),
        content: Text('Biztosan törölni szeretnéd a "${check.productName}" garanciális csekket?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: const Text('Mégse'),
    ...
    if (confirmed == true) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Garanciális csekk sikeresen törölve')),
          );
        }
      } catch (e) {
        if (mounted) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _searchQuery.isEmpty ? 'Nincs garanciális csekk' : 'Nincs találat',
            style: TextStyle(
              fontSize: 18,
              color: Colors.grey[600],
              fontWeight: FontWeight.w500,
          ...
          Text(
            _searchQuery.isEmpty 
                ? 'Kattints a + gombra az első garanciális csekk hozzáadásához vagy használd a menüt dummy adatok betöltéséhez'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Garanciális Csekkek'),
